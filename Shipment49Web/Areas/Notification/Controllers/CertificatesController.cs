...
namespace Shipment49Web.Areas.Notification.Controllers
{
    ...
    public class CertificatesController : Controller
    {
        ...
        public CertificatesController(IMenuRepository repo)
        {
            sc = repo;
        ...
        public ActionResult Index(DateTime? datefrom, DateTime? dateto, string ids = "", string Acknowledge = "", int page = 1, string sort = "DOI", string sortdir = "desc", string fleettype = "",string fleetname="", string search="", string Searchbtn="")
        {
            ...
            IEnumerable<CertificationClass> clist = null;

            if (string.IsNullOrEmpty(Searchbtn))
            {
                clist = GetTreeData(datefrom, dateto);
            }
            else
            {
                clist = GetCenoti.Invoke(sc);

                if (!string.IsNullOrEmpty(fleettype))
                {
                    string[] fleettypes = fleettype.TrimEnd(',').Split(',');
                    clist = clist.Where(x => fleettypes.Contains(x.FleetType) && (x.DOI >= datefrom && x.DOI <= dateto));
                }
                if (!string.IsNullOrEmpty(fleetname))
                {
                    string[] fleetnames = fleetname.TrimEnd(',').Split(',');
                    clist = clist.Where(x => fleetnames.Contains(x.FleetName) && (x.DOI >= datefrom && x.DOI <= dateto));
                }
                if (!string.IsNullOrEmpty(search))
                {
                    string[] searchs = search.TrimEnd(',').Split(',');
                    clist = clist.Where(x => searchs.Contains(x.VesselName) && (x.DOI >= datefrom && x.DOI <= dateto));
                }
        ...
        private IQueryable<CertificationClass> GetTreeData(DateTime? datefrom, DateTime? dateto)
        {
            var locations = sc.Certifications.Where(x => x.AlertFrequency != null && x.AcknDate == null);


            var meters = sc.Certifications.Where
            (x => x.AcknDate != null && (x.DOI >= datefrom && x.DOI <= dateto));
        ...
        private readonly Func<IMenuRepository, IEnumerable<CertificationClass>> GetCenoti = c => c.Certifications.ToList();
       ...
       new { controller = "certificates", area = "notification",datefrom, dateto, search, fleetname, fleettype, page = (int)TempData["page"] }));
        ...
        */
        ...
        new { controller = "certificates", area = "notification", Id = cm.Nid }));
