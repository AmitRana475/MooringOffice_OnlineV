...
namespace Shipment49Web.Areas.MooringTail.Controllers
{
    ...
    [ErrorClass]
    public class TailInspectionController : BaseController
    {
       ...
       // public static int VesselID { get; set; }
        ...
        public ActionResult Index(int? page)
        {
           int VesselID = Convert.ToInt32(Session["VesselSessionID"].ToString());
            MooringTailInspections model = new MooringTailInspections();
            string year = DateTime.Now.Year.ToString();

            model.RopeInspectionList = CommonClass.MooringInspectionList1(year,1,VesselID);
        ...
        [HttpPost]
        public ActionResult Index(MooringTailInspections ins)
        {
            int VesselID = Convert.ToInt32(Session["VesselSessionID"].ToString());
            MooringTailInspections model = new MooringTailInspections();
            model.RopeInspectionList = CommonClass.MooringInspectionList1(ins.Years,1,VesselID);
            model.YearList = CommonClass.GetYearsList();
            return View(model);
        }
        ...
        public ActionResult addlineinspection()
        {
            ...
            assR.AddInspectionList = CommonClass.AddLineInsList(1,VesselID);
            return View(assR);
        }
        //[HttpPost]
        ...
        [HttpPost]
        [PreventSpam("InsertInspectionTail", 1, 1)]
        public ActionResult InsertInspection(string inspections)
        {
            try
            {
                ...
                foreach (var rootObject in Json)
                {
                    ...
                    if (rootObject.Photo1 != "")
                    {
                        ...
                        if (str1.Contains(FileExtension))
                        {
                            ...
                            if (compare > 2000000)
                            {
                                ...
                                return RedirectToAction(Url.Action("Index", "TailInspection"));
                            }
                    ...
                    using (SqlDataAdapter adp = new SqlDataAdapter("InsertInspection", con))
                    {
                        ...
                        adp.SelectCommand.Parameters.AddWithValue("@ExternalRating_B", 0.00);
                        adp.SelectCommand.Parameters.AddWithValue("@InternalRating_B", 0.00);
                        adp.SelectCommand.Parameters.AddWithValue("@AverageRating_B", 0.00);
                        adp.SelectCommand.Parameters.AddWithValue("@LengthOFAbrasion_B", 0.00);
                        adp.SelectCommand.Parameters.AddWithValue("@DistanceOutboard_B", 0.00);
                        adp.SelectCommand.Parameters.AddWithValue("@CutYarnCount_B", 0.00);
                        adp.SelectCommand.Parameters.AddWithValue("@LengthOFGlazing_B", 0.00);
           ...
           // RedirectToAction("Index");
            ...
            return Json(Url.Action("Index", "TailInspection"));
        ...
        private void updateInspectionDueDate(int ropeid, int vesseid,DateTime? inspectdt)
        {
            ...
            {
                int VesselID = Convert.ToInt32(Session["VesselSessionID"].ToString());
                var Ropelist = context.MooringRopeDetails.Where(x => x.DeleteStatus == false && x.OutofServiceDate == null && x.VesselID==vesseid && x.RopeTail == 1).ToList();
                ...
                var InspecSetting = context.tblRopeTailInspectionSettings.ToList();
                ...
                if (ratingcheck != null)
                {
                    try
                    {
                        ...
                        using (SqlDataAdapter pp = new SqlDataAdapter("select " + rating + " from tblRopeTailInspectionSetting where mooringropetype=" + RopeId.RopeTypeId + " and manufacturertype=" + RopeId.ManufacturerId + "", con))
                        {
                        ...
                        using (SqlDataAdapter adp = new SqlDataAdapter("update MooringRopeDetail set InspectionDueDate='" + notidueMonth + "' where RopeId=" + ropeid + " and VesselID=" + vesseid + "", con))
                        {
                            DataTable dt = new DataTable();
                            adp.Fill(dt);
                        }
        ...
        [HttpPost]
        //[PreventSpam("editinspectionTail", 1, 1)]
        ...
        //[PreventSpam("DeleteinspectionTail", 3, 1)]
        public ActionResult Delete(int InspectionId)
        {
            try
            {
                int VesselID = Convert.ToInt32(Session["VesselSessionID"].ToString());
                int vesselid = Convert.ToInt32(VesselID);
                SqlDataAdapter adp1 = new SqlDataAdapter("select InspectDate from MooringRopeInspection where inspectionid='" + InspectionId + "' and IsActive=1 and RopeTail=1 and vesselId=" + vesselid + "", con);
                DataTable dt1 = new DataTable();
                adp1.Fill(dt1);
        ...
        private void UpdInspDate(int inspectionid, string inspectdate)
        {
            try
            {
                ...
                SqlDataAdapter adp = new SqlDataAdapter("select * from mooringropeinspection where InspectDate='" + inspectdate + "' and RopeTail = 1 and VesselId="+vesselid+"", con);
                DataTable dt = new DataTable();
                adp.Fill(dt);
                decimal avg = 0;
                var Ropelist = context.MooringRopeDetails.Where(x => x.DeleteStatus == false && x.OutofServiceDate == null && x.RopeTail == 1 && x.VesselID == vesselid).ToList();
                var InspecSetting = context.tblRopeTailInspectionSettings.ToList();
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    var RopeId = Ropelist.Where(x => x.RopeId == Convert.ToInt32(dt.Rows[i]["RopeId"]) && x.RopeTail == 1 && x.VesselID == vesselid).FirstOrDefault();
                    ...
                    if (ratingcheck != null)
                    {
                        try
                        {
                            ...
                            int near = Convert.ToInt32(nearest);
                            SqlDataAdapter pp = new SqlDataAdapter("select " + rating + " from tblRopeTailInspectionSetting where mooringropetype=" + RopeId.RopeTypeId + " and manufacturertype=" + RopeId.ManufacturerId + "", con);
                            System.Data.DataTable rtc = new System.Data.DataTable();
                            pp.Fill(rtc);
                            if (rtc.Rows.Count > 0)
                            {
                            ...
                            {
                                var inspectdate1 = "";
                                SqlDataAdapter adp11 = new SqlDataAdapter("select  Max(InspectDate) from MooringRopeInspection where RopeId ='" + Convert.ToInt32(dt.Rows[i]["RopeId"]) + "' and InspectionId !='" + Convert.ToInt32(dt.Rows[i]["InspectionId"]) + "' and IsActive=1 and RopeTail=1 and VesselId=" + vesselid + "", con);
                                ...
                                var result = context.MooringRopeDetails.SingleOrDefault(b => b.Id == rpid && b.IsActive == true && b.RopeTail == 1 && b.VesselID==vesselid);
                                if (result != null)
                                {
