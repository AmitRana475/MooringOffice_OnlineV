...
namespace Shipment49Web.Areas.MooringTail.Controllers
{
    ...
    [ErrorClass]
    public class TailDisposalController : BaseController
    {
       ...
       // public static string VesselID;
           ...
           int VesselID = Convert.ToInt32(Session["VesselSessionID"].ToString());
            using (ShipmentContaxt sc1 = new ShipmentContaxt())
            {
                ...
                var ilist = new ShipmentContaxt()
                     .MultipleResults("[dbo].[GetRopeDisposal]  1,'" + VesselID + "'")
                   .With<RopeDisposals>()
                   .Execute();
        ...
        public ActionResult addlinedisposal()
        {
            ...
            assR.MooringLineLists = CommonClass.MooringRopeDisposalListCommon(1,VesselID);
            assR.PortNameList = CommonClass.GetPortNameList();
            return View(assR);
        }
        [HttpPost]
        [PreventSpam("addTaildisposal", 3, 1)]
        ...
        public ActionResult Delete(int Id)
        {
            try
            {
                int VesselID = Convert.ToInt32(Session["VesselSessionID"].ToString());
                int vesselid = Convert.ToInt32(VesselID);
                SqlDataAdapter adp = new SqlDataAdapter("update RopeDisposal set IsActive='False' where Id =" + Id + " and RopeTail = 1 and VesselID='" + vesselid + "' ", con);
