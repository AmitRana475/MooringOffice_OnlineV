...
namespace Shipment49Web.Areas.MooringTail.Controllers
{
    ...
    [ErrorClass]
    public class TailSplicingController : BaseController
    {
           ...
           int VesselID = Convert.ToInt32(Session["VesselSessionID"].ToString());
            using (ShipmentContaxt sc1 = new ShipmentContaxt())
            {
                ...
                var ilist = new ShipmentContaxt()
                     .MultipleResults("[dbo].[GetRopeSplicing]  1,'" + VesselID + "'")
                   .With<RopeSplicing>()
                   .Execute();
        ...
        [HttpPost]
        [PreventSpam("addTailsplicing", 3, 1)]
        public ActionResult addlinesplicing(RopeSplicingRecord rpsplicing)
        {
            ...
            if (rpsplicing.IsLineCropped=="Yes")
            {
                if (rpsplicing.RsCropping == null)
                {
                    ...
                    rpsplicing.MooringLineLists = CommonClass.MooringRopeDetailListCommon(1,vesselid);
                    ...
                    try
                    {
                        ...
                        if (crplength >= percent)
                        {
                            ...
                            if (!string.IsNullOrEmpty(WinchName))
                            {
                                ...
                                notificationcrop = "Cropped more than 10% - RopeTail " + ropename + " - " + uniqueid + " on winch " + WinchName + " located at " + WinchLocation + "";
                            }
                            else
                            {
                                notificationcrop = "Cropped more than 10% - RopeTail " + ropename + " - " + uniqueid + "";
                            }
        ...
        public JsonResult Installdatecheck(string dtvalue, int RopeId)
        {
            ...
            {
                string msg = "Splicing Date can not less than RopeTail Installed date!";
